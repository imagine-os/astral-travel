...
for (const file of ['app.mjs', 'explorer.mjs', 'graph-styles.mjs', 'lib/examples.mjs']) {
  let source = await readFile(`dist/${file}`, 'utf8');
  for (const dependency of ['lib/core.mjs', 'explorer.mjs', 'memory-model.mjs', 'objects-3d.mjs', 'objects-css3d.mjs', 'spatial-state.mjs', 'graph-styles.mjs', 'graph-layouts.mjs', 'lib/examples.mjs']) {
    source = source.replaceAll(`'./${dependency}'`, `'./${dependency}?v=${revision}'`);
  }
  if(file==='lib/examples.mjs')source=source.replaceAll("'./core.mjs'", `'./core.mjs?v=${revision}'`);
